...
from .dexnet.grasping.visualizer3d import DexNetVisualizer3D
...
def eval_grasp(grasp_group, models, dexnet_models, obj_list, poses, config, table=None, voxel_size=0.008, TOP_K = 50):
    '''
    **Input:**
    ...
    '''
    print(len(dexgrasp_list))
    print(len(dexnet_models))
    gripper_gsp = dexgrasp_list[3][1]
    model_gsp = dexnet_models[3]
    gripper = RobotGripper.load(gripper_name = 'panda', gripper_dir = '../graspnetAPI/utils/dexnet/grasping/data/grippers')
    DexNetVisualizer3D.figure()
    print(dexnet_models)
    print(gripper_gsp)
    DexNetVisualizer3D.gripper_on_object(gripper,gripper_gsp,model_gsp)
    DexNetVisualizer3D.show()

    print("!!!!!!!!!!!!!!!!")
    '''
    idx = 9
    print(len(dexgrasp_list[idx]))
    dex_gsp = dexgrasp_list[idx][0]
    dex_gsp_1 = dexgrasp_list[idx][1]
    dex_gsp_2 = dexgrasp_list[idx][2]
    dex_gsp_3 = dexgrasp_list[idx][3]
    points = np.array([dex_gsp.center])
    points_1 = np.array([dex_gsp_1.center])
    points_2 = np.array([dex_gsp_2.center])
    points_3 = np.array([dex_gsp_3.center])

    pt2=o3d.geometry.PointCloud()
    pt2.points=o3d.utility.Vector3dVector(points.reshape(-1,3))
    pt2.paint_uniform_color([0,1,0])

    
    pt3=o3d.geometry.PointCloud()
    pt3.points=o3d.utility.Vector3dVector(points_1.reshape(-1,3))
    pt3.paint_uniform_color([0,1,0])

    
    pt4=o3d.geometry.PointCloud()
    pt4.points=o3d.utility.Vector3dVector(points_2.reshape(-1,3))
    pt4.paint_uniform_color([0,1,0])

   
    pt5=o3d.geometry.PointCloud()
    pt5.points=o3d.utility.Vector3dVector(points_3.reshape(-1,3))
    pt5.paint_uniform_color([0,1,0])
    


    print(obj_list[idx])
    file_dir = f'{os.path.expanduser('~')}/dataset/graspnet/models/'+ '%03d' % obj_list[idx] +'/nontextured.ply'  #文件的路径
    pcd = o3d.io.read_point_cloud(file_dir)

    
    print("==========================")
    #pcd = o3d.utility.Vector3dVector(left_points)
    
    o3d.visualization.draw_geometries([pcd, pt2, pt3,pt4,pt5])
