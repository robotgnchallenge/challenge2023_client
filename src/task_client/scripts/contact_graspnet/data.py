...
def subscribe_pc_ros( cam_mode, rgb_topic, pc_topic):
    #rospy.init_node('subscribe_pc_ros')

    segmap, rgb, depth, pc_full, pc_colors = None, None, None, [], []
    ...
    pc = pc2.read_points(pc_sim, field_names=("x","y","z"), skip_nans=True)
    ...
    if(cam_mode == "Sim"):
        ...
        for p in pc:
            ...
            pc_colors.append([rgb[v][u][2],rgb[v][u][1],rgb[v][u][0]])

    else:
        cx = 324.3047790527344
        cy = 235.1234588623047
        fx = 604.1572265625
        fy = 604.2328491210938

        pc = rospy.wait_for_message(pc_topic,PointCloud2)
        pc_point = pc2.read_points(pc, field_names = ("x", "y", "z", "rgb"))

        pc_color = ros_numpy.numpify(pc)
        pc_color = ros_numpy.point_cloud2.split_rgb_field(pc_color)
        pc_x = pc_color['x'].reshape(pc_color['x'].shape[0] * pc_color['x'].shape[1])
        pc_y = pc_color['y'].reshape(pc_color['y'].shape[0] * pc_color['y'].shape[1])
        pc_z = pc_color['z'].reshape(pc_color['z'].shape[0] * pc_color['z'].shape[1])
        pc_r = pc_color['r'].reshape(pc_color['r'].shape[0] * pc_color['r'].shape[1])
        pc_g = pc_color['g'].reshape(pc_color['g'].shape[0] * pc_color['g'].shape[1])
        pc_b = pc_color['b'].reshape(pc_color['b'].shape[0] * pc_color['b'].shape[1])

        i = 0
        while(i < pc_x.shape[0]):
            if(math.isnan(pc_x[i])):
                pass
            else:
                pc_full.append([pc_x[i],pc_y[i],pc_z[i]])
                pc_colors.append([pc_r[i],pc_g[i],pc_b[i]])
            i += 3
    ...
    pc_colors = np.array(pc_colors)

    sample_pts = 20000
    choice = np.random.choice(len(pc_full), sample_pts, replace=False if len(pc_full) >= sample_pts else True)
    pc_full = pc_full[choice]
    pc_colors = pc_colors[choice]
